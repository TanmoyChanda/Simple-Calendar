    ...
    stages{
		...
		stage('Sign Apk and archive') {
			steps{
				...
				shell('''
					|cd target
					|curl -v -F r=maven-releases -F hasPom=false -F e=apk -F -F p=apk -F file=@app/build/outputs/apk/release/calendar-release${BUILD_NUMBER}.apk -u admin:admin123 http://13.127.211.17:8081/nexus/content/repositories/central/
					'''.stripMargin()
					)
				}
			}	
